...
class Kalman_Filtering:
    ...
    def predict_only(self, dt):
        """Wykonaj tylko predykcję bez korekty (gdy brak pomiarów)"""
        # Aktualizuj macierz przejścia z nowym dt
        for i, j in zip(self.Measurement_array, self.dt_array):
            self.kalman.transitionMatrix[i, j] = dt
        pred = []
        # Tylko krok predykcji, bez correction
        tp = self.kalman.predict()
